...
from .models import TitleLink,Title,Login
...
def login(request_client):
    # Use the proper method to access POST data
    username = request_client.POST.get('Username', '')
    password = request_client.POST.get('Password', '')

    # Use the correct model and field names
    login_credentials = Login.objects.filter(Username=username, Password=password)

    if login_credentials.exists():
        is_checker = 'yes'
    else:
        is_checker = 'no'
...
def view_archives(request_client):
    try:
        ...
        relay1=[title_link.relay1 for title_link in title_links]
        relay2=[title_link.relay2 for title_link in title_links]
        relay3=[title_link.relay3 for title_link in title_links]
        print(image_links)
        # Create a JSON response with the image links
        response_data = {'image_links': image_links,'timestamps':timestamps,'relay1':relay1,'relay2':relay2,'relay3':relay3}
        return JsonResponse(response_data)
    ...
    try:
        ...
        if image_urls_onion:
            ...
            for img in imgurls:
                ...
                upload_photo(f"downloaded{i}.png",f"user_input{i}")
                i+=1
            tor_process.kill()
            return JsonResponse( responsedata )
        else:
...
SERVICE_ACCOUTNT_FILE=str(os.getenv("SERVICE_ACCOUTNT_FILE"))
...
def upload_photo(file_path,user_input):
...
def circuit_info():
    with Controller.from_port() as controller:
        ...
        time.sleep(10)
        relays = ["Not Found","Not Found","Not Found"]
        try:
...
upload_location = str(os.getenv('upload_location'))
